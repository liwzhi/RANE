...
class mode_evaludatoin():
    def __init__(self, model, G, embedding_size):
        self.model = model
        self.G = G
    ...
    def clustering_embedding(self, labels, embedding_size):
        ...
        return
    def mutli_lables(self, labels, embedding_size, clf = LogisticRegression( C= 1, penalty = "l2", tol=0.01),
                     flag = "superviseddddd"):
    ...
    def model_evaludate(self, labels,clf, embedding_size, flag = "supervised"):
        G = self.G
        model = self.model
        a =  G.nodes()
        print "#######"
        ...
        for node in G.nodes():
            try:
                ...
                vec_one = np.random.rand(128)
                un_seen_node +=1
            X[count, :] = vec_one
            count +=1
            label_get = labels.loc[labels['nodes'] == node]
            previou = 0
            if not label_get.empty:
                label_get = label_get["label"].iloc[0]
                previou = label_get
            else:
                label_get = previou
                un_seen_node +=1
            labels_nodes.append(label_get)
        ...
        for item in items:
            X_train, X_test, y_train, y_test = train_test_split(X, labels_nodes, test_size= item, random_state=51)
            if flag == "supervised":
            ...
            list_mico.append(round(micro_f1,4))
            list_maco.append(round(macro_f2,4))
        print list_mico
        print list_maco
